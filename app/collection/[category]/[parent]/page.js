...
export default async function Parents(parent) {
  ...
  return (
    <div
      key={parentItem.id}
      className="flex flex-col justify-between items-center text-sm mb-1 w-full bg-neutral-100 text-neutral-900 h-fit"
    >
      ...
      <h1 className="flex-none text-lg md:mb-12 border border-x-0 border-3 border-black rounded-lg md:px-6 md:py-4">
        {parentItem.name}
      </h1>
      <p className="h-24 text-center mt-4 md:mt-2">{parentItem.description}</p>
